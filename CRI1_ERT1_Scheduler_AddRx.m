...
function [num_chargers, Testing_Schedule, Testing_Summary, note] = CRI1_ERT1_Scheduler_AddRx(number_of_days, number_of_chargers, number_of_Rx) 
...
%the start of reduction of charging time is after the first cycle with 5
%iterations of chargin (3hr:45mins) and 10 cycles of testing which is the
%first 4 days in total
    number_of_days=number_of_days-4

%additional code for increasing number of Rx
    number_of_Rx = 9-(number_of_Rx-9)

%Charging Time Minimization
    num_Rx = optimvar('num_Rx', 'type', 'integer', 'LowerBound',number_of_Rx, 'UpperBound', number_of_Rx);
...
%--------------------------------------------------
    ...
    for num_day = 1:TotalTest_Days
        while time < 60*8
            if Charging == 0
                ...
                prob.Constraints.c8 = charging_time + x7+d7 - 40 + 10 <= 60*8 - time;
            ...
            %-------------------------------------------------------------------------------------------------
                ...
                prob.Constraints.c8 = x7+d7<= 60*8-time;
                ...
                if (60*8-time) <= (40+10+20+10+8+15) || (60*8-time) <= charging_time
                    break
                end
    ...
    note = sprintf('With a total of %4.0f tests for %4.0f days', (Total_Tests+10),(Days+4));
end